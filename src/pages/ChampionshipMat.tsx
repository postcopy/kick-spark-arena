...
function ChampionshipMatInner() {
  ...
  useEffect(() => {
    ...
    const winnerSide: 'RED' | 'BLUE' = sync.state.roundsWonRed > sync.state.roundsWonBlue ? 'RED' : 'BLUE';
  ...
  const handleOpenTV = () => {
    tvWindowRef.current = window.open(
      `${window.location.pathname}#/championship/tv?mat=${matId}`,
  ...
  const handleNextMatch = useCallback(() => {
    ...
    for (const cat of tournamentHook.tournament.categories) {
      ...
      if (next) {
        ...
        sync.saveConfig({
          ...currentConfig,
          athleteRedName: next.athleteRed?.name || '',
          athleteBlueName: next.athleteBlue?.name || '',
        });
  ...
  const handleGoToTournament = useCallback(() => {
    navigate('/championship/setup');
  }, [navigate]);

  const handleOverrideWinner = useCallback(() => {
    // Simple: flip the winner in the tournament bracket
    if (!tournamentHook.tournament?.currentCategoryId || !tournamentHook.tournament?.currentMatchId) return;
    const newWinner: 'RED' | 'BLUE' = sync.state.roundsWonRed > sync.state.roundsWonBlue ? 'BLUE' : 'RED';
    tournamentHook.overrideMatchWinner(
      tournamentHook.tournament.currentCategoryId,
      tournamentHook.tournament.currentMatchId,
      newWinner,
    );
  }, [tournamentHook, sync.state.roundsWonRed, sync.state.roundsWonBlue]);
  ...
  return (
    <div className="h-screen flex bg-[hsl(var(--sulsport-black))]">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        {/* Header */}
        <header className="h-14 bg-[hsl(var(--sulsport-dark))] border-b border-[hsl(var(--sulsport-gray))] flex items-center justify-between px-6 relative">
        ...
        {/* Tournament: Next Match Bar */}
        {hasTournament && tournamentHook.tournament && sync.state.status === 'MATCH_END' && (
          <div className="shrink-0">
            <NextMatchBar
              tournament={tournamentHook.tournament}
              currentMatchWinnerSide={sync.state.roundsWonRed > sync.state.roundsWonBlue ? 'RED' : 'BLUE'}
